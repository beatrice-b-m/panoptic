...
class TestCrPreservation:
    """Regression tests: \\r at the end of %output content must not be stripped.

    rstrip('\\n') is correct — the PTY line terminator is bare \\n (raw mode has
    OPOST disabled so tmux never emits \\r\\n).  Any \\r in the line is content
    written by the application to move the cursor to column 0.  Stripping it
    silently drops the carriage return, causing powerlevel10k segments and TUI
    applications (like OMP) to draw in the wrong column instead of redrawing
    in-place.
    """

    def test_trailing_cr_in_output_preserved(self):
        """\\r immediately before the protocol \\n must survive into the output event."""
        bridge = _make_bridge()
        # Feed a %output line whose content is a lone \r (literal 0x0D) then
        # the protocol \n (0x0A). This is the exact byte sequence tmux sends
        # when an application writes \r to move the cursor to column 0.
        lines = ["%output %0 \r"]
        events = asyncio.run(_feed_lines(bridge, lines))
        assert len(events) == 1
        ev = events[0]
        assert ev["type"] == "output"
        assert ev["data"] == b"\r", (
            "trailing \\r was stripped — rstrip is too aggressive"
        )

    def test_cr_at_end_of_output_content_preserved(self):
        """\\r after other content at end of %output must survive."""
        bridge = _make_bridge()
        lines = ["%output %0 hello\r"]
        events = asyncio.run(_feed_lines(bridge, lines))
        assert len(events) == 1
        assert events[0]["data"] == b"hello\r"

    def test_cr_in_middle_of_output_unaffected(self):
        """\\r in the middle of content (e.g. cursor-to-col-0 then redraw) is preserved."""
        bridge = _make_bridge()
        lines = ["%output %0 \rcontent"]
        events = asyncio.run(_feed_lines(bridge, lines))
        assert len(events) == 1
        assert events[0]["data"] == b"\rcontent"

    def test_normal_output_without_cr_unaffected(self):
        """Non-CR-terminated output still works correctly."""
        bridge = _make_bridge()
        lines = ["%output %0 hello"]
        events = asyncio.run(_feed_lines(bridge, lines))
        assert len(events) == 1
